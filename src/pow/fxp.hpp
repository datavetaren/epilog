...
// #define USE_EPILOG_FXP_ASSERT

#ifdef USE_EPILOG_FXP_ASSERT
#define EPILOG_FXP_ASSERT(x) { fxp_assert(x); }
#else
#define EPILOG_FXP_ASSERT(x)
#endif
...
public:
	...
	EPILOG_FXP_ASSERT(overflow_check(to_int()*3));
	if ((*this) > limit_max) {
	    return fxp1648::max();
    ...
    inline fxp1648 operator << (size_t n) const {
	EPILOG_FXP_ASSERT(overflow_check(to_int()<<1));
	...
	EPILOG_FXP_ASSERT(overflow_check(to_int() + other.to_int()));
	...
	if (scale2 > 0) {
	    EPILOG_FXP_ASSERT((msb(x.data_) + scale2) < 63);

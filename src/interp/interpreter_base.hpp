...
namespace std {
    template<> struct hash<epilog::interp::qname> {
	size_t operator()(const epilog::interp::qname &k) const {
	    return k.first.raw_value() + 
		   17*k.second.raw_value();
	}
    };
    template<> struct hash<epilog::interp::functor_index> {
	size_t operator()(const epilog::interp::functor_index &k) const {
